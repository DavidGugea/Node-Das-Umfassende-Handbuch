import { getAll, remove, get, save } from './model.js.js';
import { render as form } from './form.js.js';
...
export async function listAction(request, response) {
    try {
        ...
        response.format({
            xml() {
                moviesResponse.movies = moviesResponse.movies.map((movie) => ({movie,}));
...
export async function createAction(request, response) {
    try {
        ...
        console.error(e);
        response.status(200).json('An error happened');
    }
}
...
export async function updateAction(request, response) {
    try {
        ...
        const updatedMovie = await model.save(movie, request.user.id);
